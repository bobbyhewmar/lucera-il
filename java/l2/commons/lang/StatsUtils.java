...
public final class StatsUtils
{
	...
	public static CharSequence getMemUsage()
	{
		...
		list.append("     Allocated: .......... ").append((int) allocatedMem).append(" KB (").append((double) Math.round(allocatedMem / maxMem * 1000000.0) / 10000.0).append("%)").append("\n");
		double nonAllocatedMem = maxMem - allocatedMem;
		list.append("     Non-Allocated: ...... ").append((int) nonAllocatedMem).append(" KB (").append((double) Math.round(nonAllocatedMem / maxMem * 1000000.0) / 10000.0).append("%)").append("\n");
		list.append("AllocatedMemory: ......... ").append((int) allocatedMem).append(" KB").append("\n");
		list.append("     Used: ............... ").append((int) usedMem).append(" KB (").append((double) Math.round(usedMem / maxMem * 1000000.0) / 10000.0).append("%)").append("\n");
		double cachedMem = allocatedMem - usedMem;
		list.append("     Unused (cached): .... ").append((int) cachedMem).append(" KB (").append((double) Math.round(cachedMem / maxMem * 1000000.0) / 10000.0).append("%)").append("\n");
		double useableMem = maxMem - usedMem;
		list.append("UseableMemory: ........... ").append((int) useableMem).append(" KB (").append((double) Math.round(useableMem / maxMem * 1000000.0) / 10000.0).append("%)").append("\n");
		return list;
	}
