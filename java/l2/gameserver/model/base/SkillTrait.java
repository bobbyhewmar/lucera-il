...
public enum SkillTrait
{
	...
	DERANGEMENT
			{
				...
				public final double calcProf(Env env)
				{
					return Math.min(40.0, env.character.calcStat(Stats.MENTAL_POWER, env.target, env.skill) + calcEnchantMod(env));
				}
			},
	ETC,
	GUST,
	...
	SHOCK
			{
				...
				public final double calcProf(Env env)
				{
					return Math.min(40.0, env.character.calcStat(Stats.STUN_POWER, env.target, env.skill) + calcEnchantMod(env));
				}
			},
	SLEEP
			{
	...
	public static double calcEnchantMod(Env env)
	{
		...
		return env.skill.getEnchantLevelCount() == 15 ? enchantLevel * 2.0 : enchantLevel;
	}
