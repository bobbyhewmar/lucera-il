...
{
	public static final long CYCLE_TIME = 3600000;
	...
	public abstract void changeOwner(Clan newOwner);
	...
	protected void loadFunctions()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				function.setEndTimeInMillis((long) rs.getInt("endTime") * 1000);
	...
	public boolean updateFunctions(int type, int level)
	{
		...
		try
		{
			...
			{
				if(count >= (long) (lease - getFunction(type).getLease()))
				{
	...
	public void startCycleTask()
	{
		...
		long diff;
		for(diff = System.currentTimeMillis() - ownedTime;diff >= 3600000;diff -= 3600000)
		{
		}
		_cycleTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new ResidenceCycleTask(), diff, 3600000);
	}
