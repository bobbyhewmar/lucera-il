...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.startsWith("admin_show_characters_by_ip"))
		{
			try
			{
				String sciArgsTxt = fullString.substring(28).trim();
				String[] sciArgs = sciArgsTxt.split("\\s+");
		...
		else
		{
			...
			else if(fullString.equals("admin_rec"))
			{
				...
				Player player = (Player) target;
				player.setGivableRec(player.getGivableRec() + 1);
			...
			else if(fullString.startsWith("admin_rec"))
			{
				try
				{
					...
					player.setGivableRec(player.getGivableRec() + recVal);
					player.sendMessage("You have been recommended by a GM");
					player.broadcastCharInfo();
				}
			...
			else if(fullString.startsWith("admin_add_exp_sp"))
			{
				try
				{
					...
					long exp = NumberUtils.toLong(vals[0], (long) 0);
			...
			else if(fullString.startsWith("admin_remove_item"))
			{
				...
				if(wordList.length >= 3)
				{
					...
					if(target != null && id > 0 && count > 0)
					{
						...
						{
							help = true;
							activeChar.sendMessage("Removed " + ItemFunctions.removeItem(target, id, count, true) + " from '" + target.getName() + "'");
						}
					}
				}
				if(!help)
	...
	private void listCharactersByIp(Player activeChar, String IP, int page)
	{
		...
		{
			String playerIp;
			if(player == null || player.isInOfflineMode() || !player.isConnected() || player.getNetConnection() == null || (playerIp = player.getNetConnection().getIpAddr()) == null || !IP.trim().equals(playerIp))
				continue;
			players.add(player);
		}
		...
		{
			int pagenr = x + 1;
			replyMSG.append("<center><a action=\"bypass -h admin_show_characters_by_ip " + IP + " " + x + "\">Page " + pagenr + "</a></center>");
		}
		replyMSG.append("<br>");
		replyMSG.append("<table width=270>");
