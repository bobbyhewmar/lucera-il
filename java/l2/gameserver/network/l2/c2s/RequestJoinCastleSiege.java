...
public class RequestJoinCastleSiege extends L2GameClientPacket
{
	...
	private static void registerAtCastle(Player player, Castle castle, boolean attacker, boolean join)
	{
		...
		if(join)
		{
			...
			{
				Object tempCastle = residence.getSiegeEvent().getSiegeClan("attackers", playerClan);
				if(tempCastle == null)
				{
			...
			if(attacker)
			{
				...
				siegeClan = new SiegeClanObject("attackers", playerClan, 0);
			...
			else
			{
				...
				siegeClan = new SiegeClanObject("defenders_waiting", playerClan, 0);
