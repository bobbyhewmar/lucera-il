...
public class OnActionShift extends Functions
{
	public boolean OnActionShift_NpcInstance(Player player, GameObject object)
	{
		...
		if(object.isNpc())
		{
			...
			if(Config.ALT_FULL_NPC_STATS_PAGE)
			{
				...
				{
					HardSpawner hardSpawner = (HardSpawner) npc.getSpawn();
					...
					StringBuilder b = new StringBuilder(hardSpawner.getTemplate().getEventName() + "|");
				...
				dialog = dialog.replaceFirst("%respawn%", npc.getSpawn() != null ? Util.formatTime((int) (npc.getSpawn().getRespawnCron() != null ? (npc.getSpawn().getRespawnCron().next(System.currentTimeMillis()) - System.currentTimeMillis()) / 1000 : npc.getSpawn().getRespawnDelay())) : "0");
			...
			dialog = player.isGM() ? dialog.replaceFirst("%AI%", npc.getAI() + ",<br1>active: " + npc.getAI().isActive() + ",<br1>intention: " + npc.getAI().getIntention()) : dialog.replaceFirst("%AI%", "");
			show(dialog, player, npc, (Object[]) new Object[0]);
		}
		return true;
	}
	...
	public void quests()
	{
		...
		show(dialog.toString(), player, npc, (Object[]) new Object[0]);
	}
	...
	public void aggro()
	{
		...
		TreeSet<AggroList.HateInfo> set = new TreeSet<>(AggroList.HateComparator.getInstance());
