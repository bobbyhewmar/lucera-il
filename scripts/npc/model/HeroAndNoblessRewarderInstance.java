...
{
	public HeroAndNoblessRewarderInstance(int objectId, NpcTemplate template)
	{
	...
	protected void onDeath(Creature killer)
	{
		...
		if(killer != null && killer.isPlayable())
		{
			...
			if(Config.ALT_ALLOW_CUSTOM_HERO)
			{
				for(Player partyPlayer : playerParty)
				{
					if(partyPlayer == null || partyPlayer.isDead() || partyPlayer.isSubClassActive() || partyPlayer.getLevel() < 76 || getDistance3D(partyPlayer) > (double) Config.ALT_PARTY_DISTRIBUTION_RANGE)
						continue;
