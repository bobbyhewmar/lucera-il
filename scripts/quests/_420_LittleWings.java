...
public class _420_LittleWings extends Quest implements ScriptFile
{
	...
	private static final int[][] Fairy_Stone_Items = {{Coal, 10}, {Charcoal, 10}, {GemstoneD, 1}, {Silver_Nugget, 3}, {Toad_Lord_Back_Skin, 10}};
	private static final int[][] Delux_Fairy_Stone_Items = {{Coal, 10}, {Charcoal, 10}, {GemstoneC, 1}, {Stone_of_Purity, 1}, {Silver_Nugget, 5}, {Toad_Lord_Back_Skin, 20}};
	...
	public _420_LittleWings()
	{
		...
		addTalkId(30608);
		addKillId(20231);
		int Enchanted_Valey_id = 20589;
		while(Enchanted_Valey_id <= 20599)
		{
	...
	public static boolean CheckFairyStoneItems(QuestState st, int[][] item_list)
	{
		...
		{
			if(st.getQuestItemsCount(_item[0]) >= (long) _item[1])
				continue;
			return false;
		}
		return true;
	...
	public static void TakeFairyStoneItems(QuestState st, int[][] item_list)
	{
		for(int[] _item : item_list)
		{
			st.takeItems(_item[0], (long) _item[1]);
		}
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30608-03.htm") && _state == 2 && cond == 2 && st.getQuestItemsCount(Fairy_Stone_List) > 0)
		{
		...
		else if(event.equalsIgnoreCase("30608-03a.htm") && _state == 2 && cond == 2 && st.getQuestItemsCount(Deluxe_Fairy_Stone_List) > 0)
		{
		...
		{
			if(event.equalsIgnoreCase("30711-03.htm") && _state == 2 && cond == 3 && st.getQuestItemsCount(Fairy_Stone) + st.getQuestItemsCount(Deluxe_Fairy_Stone) > 0)
			{
				st.setCond(4);
				st.playSound("ItemSound.quest_middle");
				if(st.getQuestItemsCount(Deluxe_Fairy_Stone) > 0)
				{
					if(st.getInt("broken") != 1)
						return "30711-03a.htm";
					return "30711-04a.htm";
				}
				if(st.getInt("broken") != 1)
					return event;
				return "30711-04.htm";
			}
			if(event.equalsIgnoreCase("30747-02.htm") && _state == 2 && cond == 4 && st.getQuestItemsCount(Fairy_Stone) > 0)
			{
				st.takeItems(Fairy_Stone, -1);
				st.set("takedStone", "1");
				return event;
			}
			else if(event.equalsIgnoreCase("30747-02a.htm") && _state == 2 && cond == 4 && st.getQuestItemsCount(Deluxe_Fairy_Stone) > 0)
			{
				st.takeItems(Deluxe_Fairy_Stone, -1);
				st.set("takedStone", "2");
			...
			else if(event.equalsIgnoreCase("30748-02.htm") && cond == 5 && _state == 2 && st.getQuestItemsCount(Juice_of_Monkshood) > 0)
			{
				st.setCond(6);
			...
			else if(event.equalsIgnoreCase("30749-02.htm") && cond == 5 && _state == 2 && st.getQuestItemsCount(Juice_of_Monkshood) > 0)
			{
				st.setCond(6);
				st.takeItems(Juice_of_Monkshood, -1);
				st.giveItems(3824, 1);
			...
			else if(event.equalsIgnoreCase("30750-02.htm") && cond == 5 && _state == 2 && st.getQuestItemsCount(Juice_of_Monkshood) > 0)
			{
				st.setCond(6);
				st.takeItems(Juice_of_Monkshood, -1);
				st.giveItems(3826, 1);
			...
			else if(event.equalsIgnoreCase("30751-02.htm") && cond == 5 && _state == 2 && st.getQuestItemsCount(Juice_of_Monkshood) > 0)
			{
				st.setCond(6);
				st.takeItems(Juice_of_Monkshood, -1);
				st.giveItems(3828, 1);
			...
			else if(event.equalsIgnoreCase("30752-02.htm") && cond == 5 && _state == 2 && st.getQuestItemsCount(Juice_of_Monkshood) > 0)
			{
				st.setCond(6);
				st.takeItems(Juice_of_Monkshood, -1);
				st.giveItems(3830, 1);
			...
			{
				if(!event.equalsIgnoreCase("30747-11.htm"))
					return event;
				if(_state != 2)
					return event;
				if(cond != 7)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2 && npcId == 20231)
		{
			...
			if(st.getQuestItemsCount(Toad_Lord_Back_Skin) < (long) needed_skins && Rnd.chance(30))
			{
				st.giveItems(Toad_Lord_Back_Skin, 1);
				st.playSound(st.getQuestItemsCount(Toad_Lord_Back_Skin) < (long) needed_skins ? "ItemSound.quest_itemget" : "ItemSound.quest_middle");
			}
			return null;
		}
		if(npcId >= 20589 && npcId <= 20599 && st.getQuestItemsCount(Deluxe_Fairy_Stone) > 0)
		{
			st.takeItems(Deluxe_Fairy_Stone, 1);
			st.set("broken", "1");
			st.setCond(1);
		...
		int wyrm_id;
		if(cond == 6 && (wyrm_id = isWyrmStoler(npcId)) > 0 && st.getQuestItemsCount(getWyrmScale(wyrm_id)) > 0 && st.getQuestItemsCount(getWyrmEgg(wyrm_id)) < 20 && Rnd.chance(50))
		{
			st.giveItems(getWyrmEgg(wyrm_id), 1);
			st.playSound(st.getQuestItemsCount(getWyrmEgg(wyrm_id)) < 20 ? "ItemSound.quest_itemget" : "ItemSound.quest_middle");
		}
		return null;
	}
