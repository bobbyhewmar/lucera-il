...
public class _421_LittleWingAdventures extends Quest implements ScriptFile
{
	...
	private static final int Dragonflute_of_Wind = PetDataTable.L2Pet.HATCHLING_WIND.getControlItemId();
	private static final int Dragonflute_of_Star = PetDataTable.L2Pet.HATCHLING_STAR.getControlItemId();
	private static final int Dragonflute_of_Twilight = PetDataTable.L2Pet.HATCHLING_TWILIGHT.getControlItemId();
	private static final int Dragon_Bugle_of_Wind = PetDataTable.L2Pet.STRIDER_WIND.getControlItemId();
	private static final int Dragon_Bugle_of_Star = PetDataTable.L2Pet.STRIDER_STAR.getControlItemId();
	private static final int Dragon_Bugle_of_Twilight = PetDataTable.L2Pet.STRIDER_TWILIGHT.getControlItemId();
	...
	{
		ArrayList<ItemInstance> Dragonflutes = new ArrayList<>();
	...
	private static boolean HatchlingSummoned(QuestState st, boolean CheckObjID)
	{
		...
		ItemInstance dragonflute;
		if((dragonflute = GetDragonflute(st)) == null)
		{
			return false;
		}
	...
	{
		if(st.getState() != 2 || st.getCond() != 2 || !HatchlingSummoned(st, true) || st.getQuestItemsCount(Fairy_Leaf) == 0)
		{
			return null;
		}
