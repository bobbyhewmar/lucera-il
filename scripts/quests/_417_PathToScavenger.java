...
public class _417_PathToScavenger extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.set("id", "0");
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 53 && st.getQuestItemsCount(RING_OF_RAVEN) == 0)
			{
				st.setCond(1);
				st.setState(2);
			...
			{
				htmltext = st.getPlayer().getClassId().getId() == 54 ? "collector_pipi_q0417_02a.htm" : "collector_pipi_q0417_08.htm";
			}
			else if(st.getPlayer().getLevel() < 18 && st.getPlayer().getClassId().getId() == 53)
			{
				htmltext = "collector_pipi_q0417_02.htm";
			}
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 53 && st.getQuestItemsCount(RING_OF_RAVEN) == 1)
			{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Pippi)
		{
			...
			else if(st.getQuestItemsCount(PIPIS_LETTER) == 0 && id == 2)
			{
				htmltext = "collector_pipi_q0417_01.htm";
			}
			else if(st.getQuestItemsCount(PIPIS_LETTER) == 0)
		...
		else
		{
			...
			else if(npcId == Shari)
			{
				...
				{
					htmltext = st.getInt("id") < 20 ? "trader_chali_q0417_01.htm" : "trader_chali_q0417_02.htm";
					st.takeItems(CHARIS_AXE, 1);
			...
			else if(npcId == Bronk)
			{
				...
				{
					htmltext = st.getInt("id") < 20 ? "head_blacksmith_bronk_q0417_01.htm" : "head_blacksmith_bronk_q0417_02.htm";
					st.takeItems(BRONKS_INGOT, 1);
			...
			else if(npcId == Zimenf)
			{
				...
				{
					htmltext = st.getInt("id") < 20 ? "zimenf_priest_of_earth_q0417_01.htm" : "zimenf_priest_of_earth_q0417_02.htm";
					st.takeItems(ZIMENFS_POTION, 1);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond && st.getQuestItemsCount(BEAR_PIC) == 1 && st.getQuestItemsCount(HONEY_JAR) < 5 && mob.isSpoiled())
			{
		...
		else if((npcId == HunterTarantula || npcId == PlunderTarantula) && cond && st.getQuestItemsCount(TARANTULA_PIC) == 1 && st.getQuestItemsCount(BEAD) < 20 && mob.isSpoiled() && Rnd.chance(50))
		{
			st.giveItems(BEAD, 1);
