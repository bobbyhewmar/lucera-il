...
public class _325_GrimCollector extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("samed_q0325_06.htm"))
		{
			if(pieces(st) > 0)
			{
				st.giveItems(57, 30 * st.getQuestItemsCount(ZOMBIE_HEAD) + 20 * st.getQuestItemsCount(ZOMBIE_HEART) + 20 * st.getQuestItemsCount(ZOMBIE_LIVER) + 50 * st.getQuestItemsCount(SKULL) + 15 * st.getQuestItemsCount(RIB_BONE) + 10 * st.getQuestItemsCount(SPINE) + 10 * st.getQuestItemsCount(ARM_BONE) + 10 * st.getQuestItemsCount(THIGH_BONE) + 2000 * st.getQuestItemsCount(COMPLETE_SKELETON));
		...
		{
			st.giveItems(57, 2000 * st.getQuestItemsCount(COMPLETE_SKELETON));
			st.takeItems(COMPLETE_SKELETON, -1);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			htmltext = st.getQuestItemsCount(ANATOMY_DIAGRAM) == 0 ? "guard_curtiz_q0325_04.htm" : "guard_curtiz_q0325_05.htm";
		}
