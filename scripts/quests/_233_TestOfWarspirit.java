...
public class _233_TestOfWarspirit extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30649-03.htm") && _state == 2 && st.getQuestItemsCount(WARSPIRIT_TOTEM) > 0)
		{
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		if(npcId == Porta && qs.getQuestItemsCount(ORIMS_CONTRACT) > 0 && qs.getQuestItemsCount(PORTAS_EYE) < 10)
		{
			...
			qs.playSound(qs.getQuestItemsCount(PORTAS_EYE) == 10 ? "ItemSound.quest_middle" : "ItemSound.quest_itemget");
		}
		else if(npcId == Excuro && qs.getQuestItemsCount(ORIMS_CONTRACT) > 0 && qs.getQuestItemsCount(EXCUROS_SCALE) < 10)
		{
			...
			qs.playSound(qs.getQuestItemsCount(EXCUROS_SCALE) == 10 ? "ItemSound.quest_middle" : "ItemSound.quest_itemget");
		}
		else if(npcId == Mordeo && qs.getQuestItemsCount(ORIMS_CONTRACT) > 0 && qs.getQuestItemsCount(MORDEOS_TALON) < 10)
		{
			...
			qs.playSound(qs.getQuestItemsCount(MORDEOS_TALON) == 10 ? "ItemSound.quest_middle" : "ItemSound.quest_itemget");
		}
		...
		else if(npcId == Medusa && qs.getQuestItemsCount(MANAKIAS_TOTEM) > 0)
		{
			...
			if(drops.size() > 0 && Rnd.chance(30))
			{
				int drop_id = drops.get(Rnd.get(drops.size()));
				qs.giveItems(drop_id, 1);
				qs.playSound(drops.size() == 1 && qs.getQuestItemsCount(HERMODTS_SKULL) > 0 ? "ItemSound.quest_middle" : "ItemSound.quest_itemget");
			}
			drops.clear();
		}
		...
		else if((npcId == Tamlin_Orc || npcId == Tamlin_Orc_Archer) && qs.getQuestItemsCount(VENDETTA_TOTEM) > 0 && qs.getQuestItemsCount(TAMLIN_ORC_HEAD) < 13 && Rnd.chance(npcId == Tamlin_Orc ? 30 : 50))
		{
			qs.giveItems(TAMLIN_ORC_HEAD, 1);
			qs.playSound(qs.getQuestItemsCount(TAMLIN_ORC_HEAD) == 13 ? "ItemSound.quest_middle" : "ItemSound.quest_itemget");
		}
		return null;
	}
