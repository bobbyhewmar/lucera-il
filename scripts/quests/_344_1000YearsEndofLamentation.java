...
public class _344_1000YearsEndofLamentation extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30754-06.htm") && cond == 1)
		{
			...
			{
				if((long) Rnd.get((int) (1000.0 / st.getRateQuestsReward())) >= amount)
				{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			htmltext = amount > 0 ? "30754-05.htm" : "30754-09.htm";
		}
		else if(cond == 2)
		{
			if(npcId == 30754)
	...
	private boolean rewards(QuestState st, int npcId)
	{
		...
		if(npcId == 30857 && st.getQuestItemsCount(4273) > 0)
		{
			st.set("mission", "1");
			st.takeItems(4273, -1);
			state = true;
		...
		else if(npcId == 30704 && st.getQuestItemsCount(4272) > 0)
		{
			st.set("mission", "2");
			st.takeItems(4272, -1);
			state = true;
		...
		else if(npcId == 30623 && st.getQuestItemsCount(4271) > 0)
		{
			st.set("mission", "3");
			st.takeItems(4271, -1);
			state = true;
		...
		else if(npcId == 30756 && st.getQuestItemsCount(4270) > 0)
		{
			st.set("mission", "4");
			st.takeItems(4270, -1);
			state = true;
