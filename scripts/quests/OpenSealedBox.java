...
public class OpenSealedBox
{
	...
	public OpenSealedBox(QuestState st, int count)
	{
		...
		takecount = count;
		if(st.getQuestItemsCount(7255) < (long) count)
		{
	...
	public String apply()
	{
		if(takecount > 0)
		{
			...
			{
				st.giveItems(itemId.intValue(), rewards.get(itemId).longValue(), false);
			}
		}
		rewards.clear();
		return result;
	...
	private boolean canGiveReward()
	{
		...
		for(Integer itemId : rewards.keySet())
		{
			ItemInstance item = st.getPlayer().getInventory().getItemByItemId(itemId.intValue());
	...
	public abstract static class RewardGroup
	{
		protected static void putReward(Map<Integer, Long> rewards, int item_id, long count)
		{
			...
			{
				count += rewards.remove(item_id).longValue();
			}
