...
public class FourSepulchersSpawn extends Functions implements ScriptFile
{
	...
	static
	{
		_allMobs = new ArrayList<>();
		_startHallSpawn = new Location[] {new Location(181632, -85587, -7218), new Location(179963, -88978, -7218), new Location(173217, -86132, -7218), new Location(175608, -82296, -7218)};
	...
	private static int loadSpawn(Map<Integer, List<NpcLocation>> table, int type)
	{
		...
		try
		{
			...
			while(rset1.next())
			{
				...
				ArrayList<NpcLocation> locations = new ArrayList<>();
				while(rset2.next())
				{
	...
	public static void spawnArchonOfHalisha(int npcId)
	{
		...
		if(_archonSpawned.get(npcId).booleanValue())
		{
			return;
		}
	...
	public static void spawnExecutionerOfHalisha(NpcInstance npc)
	{
		...
		NpcTemplate template = NpcHolder.getInstance().getTemplate(_victim.get(npc.getNpcId()).intValue());
	...
	public static void spawnKeyBox(NpcInstance npc)
	{
		...
		NpcTemplate template = NpcHolder.getInstance().getTemplate(_keyBoxNpc.get(npc.getNpcId()).intValue());
		SepulcherNpcInstance npc2 = new SepulcherNpcInstance(IdFactory.getInstance().getNextId(), template);
		Location loc = npc.getSpawnedLoc() != null ? npc.getSpawnedLoc() : npc.getLoc();
	...
	public static void spawnMonster(int npcId)
	{
		...
		List<NpcLocation> monsterList = Rnd.get(2) == 0 ? _physicalMonsters.get(npcId) : _magicalMonsters.get(npcId);
		if(monsterList != null)
		{
	...
	public static void spawnMysteriousBox(int npcId)
	{
		...
		NpcTemplate template = NpcHolder.getInstance().getTemplate(_mysteriousBoxSpawns.get(Integer.valueOf(npcId)).npcId);
		SepulcherNpcInstance npc = new SepulcherNpcInstance(IdFactory.getInstance().getNextId(), template);
