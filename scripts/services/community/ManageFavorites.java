	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if("bbsgetfav".equals(cmd))
		{
			...
			try
			{
				...
				tpl = HtmCache.getInstance().getNotNull("scripts/services/community/bbs_favoritetpl.htm", player);
				
				while(rset.next())
				{
					String fav = tpl.replace("%fav_title%", rset.getString("fav_title"));
					fav = fav.replace("%fav_bypass%", rset.getString("fav_bypass"));
					fav = fav.replace("%add_date%", String.format("%1$te.%1$tm.%1$tY %1$tH:%1tM", new Date((long) rset.getInt("add_date") * 1000L)));
			...
			tpl = HtmCache.getInstance().getNotNull("scripts/services/community/bbs_getfavorite.htm", player);
			tpl = tpl.replace("%FAV_LIST%", fl.toString());
			...
			if(fav != null)
			{
				...
				if(favs.length > 1)
				{
					...
					try
					{
						...
						statement.setInt(4, (int) (System.currentTimeMillis() / 1000L));
						statement.execute();
					}
