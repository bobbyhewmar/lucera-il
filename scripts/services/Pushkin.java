...
public class Pushkin extends Functions
{
	...
	public void doCrystallize()
	{
		...
		for(ItemInstance itm : inv.getItems())
		{
			if(!itm.canBeCrystallized(player))
				continue;
			ItemTemplate crystal = ItemHolder.getInstance().getTemplate(itm.getTemplate().getCrystalType().cry);
			...
			possibleEntry.addIngredient(new MultiSellIngredient(57, Math.round((double) (itm.getTemplate().getCrystalCount() * crystal.getReferencePrice()) * 0.05), 0));
